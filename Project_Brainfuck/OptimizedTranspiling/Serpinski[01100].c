...
FILE *result_file = createFile(result_folder_path,"Serpinski.txt");
int input_pointer = 0;
char *input;
int input_len = 0;
input = argv[2];
...
if(idx+(2) < 0 || idx+(4) > cellCount) {printf("insufficient cell count - currentArray"); return -1;} 
cells[idx+(2)] = cells[idx+(2)]+(4); 
/*Ending combined level 2 optimization*/
}
/*Starting combined level 2 optimization*/
...
while (cells[idx] != 0) {
/*Starting combined level 2 optimization*/
if(idx+(-2) < 0 || idx+(5) > cellCount) {printf("insufficient cell count - currentArray"); return -1;} 
cells[idx+(-2)] = cells[idx+(-2)]+(5); 
if(idx+(0) < 0 || idx+(-1) > cellCount) {printf("insufficient cell count - currentArray"); return -1;} 
cells[idx+(0)] = cells[idx+(0)]+(-1); 
/*Ending combined level 2 optimization*/
}
/*Starting combined level 2 optimization*/
if(idx+(-2) < 0 || idx+(2) > cellCount) {printf("insufficient cell count - currentArray"); return -1;} 
cells[idx+(-2)] = cells[idx+(-2)]+(2); 
if(idx+(0) < 0 || idx+(1) > cellCount) {printf("insufficient cell count - currentArray"); return -1;} 
cells[idx+(0)] = cells[idx+(0)]+(1); 
idx += (-2);
if(idx > cellCount && idx < 0) {printf("insufficient cellcount"); return -1;}
